...
/* TODO - Update for Fab-G board */
mux_sel_t MuxDesc7[] = {
	//gpio, value, type
	{38, LOW, FN_GPIO_OUTPUT}, // Output enable
	{38, HIGH, FN_GPIO_INPUT}, // Output disable
...
mux_sel_t MuxDesc8[] = {
	//gpio, value, type
	{40, LOW, FN_GPIO_OUTPUT}, // Output enable
	{40, HIGH, FN_GPIO_INPUT}, // Output disable
...
/* 
 * On Galileo Gen2, there are two types of fast mode, one using Memory Mapped IO and
 * the other using Port IO. Linux Pins 0-7 use PIO, and 10-15 use MMIO.
 * We use the FastIOInfo struct to hold the info on which ports/bits to set/clear
 * for each of the pins when in fast mode.
 */

//                             FastMode           FastBitValue    rPort             wPort
FastIOInfo FastIOInfoPin0  = { FAST_IO_MODE_MMIO, GPIO_FAST_IO0,  MMIO_GPIO_LVL_IN, MMIO_GPIO_LVL_OUT };
FastIOInfo FastIOInfoPin1  = { FAST_IO_MODE_MMIO, GPIO_FAST_IO1,  MMIO_GPIO_LVL_IN, MMIO_GPIO_LVL_OUT };
FastIOInfo FastIOInfoPin2  = { FAST_IO_MODE_MMIO, GPIO_FAST_IO2,  MMIO_GPIO_LVL_IN, MMIO_GPIO_LVL_OUT };
FastIOInfo FastIOInfoPin3  = { FAST_IO_MODE_MMIO, GPIO_FAST_IO3,  MMIO_GPIO_LVL_IN, MMIO_GPIO_LVL_OUT };
FastIOInfo FastIOInfoPin10 = { FAST_IO_MODE_MMIO, GPIO_FAST_IO10, MMIO_GPIO_LVL_IN, MMIO_GPIO_LVL_OUT };
FastIOInfo FastIOInfoPin12 = { FAST_IO_MODE_MMIO, GPIO_FAST_IO12, MMIO_GPIO_LVL_IN, MMIO_GPIO_LVL_OUT };
...
PinDescription AltPin5 =
	{ 0,	NONE,	&FastIOInfoPin5,	5,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc5,	MUX_SIZE(MuxDesc5),	FN_GPIO,	-1,	1,	0,	NULL	 };	// Arduino IO5 or PWM pin #2
PinDescription AltPin6 =															        
	{ 1,	NONE,	&FastIOInfoPin6,	6,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc6,	MUX_SIZE(MuxDesc6),	FN_GPIO,	-1,	1,	0,	NULL	 };	// Arduino IO6 or PWM pin #3
PinDescription AltPin7 =															        
	{ 2,	NONE,	&FastIOInfoPin7,	7,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc7,	MUX_SIZE(MuxDesc7),	FN_GPIO,	-1,	1,	0,	NULL	 };	// Arduino IO7
PinDescription AltPin8 =															        
	{ 3,	NONE,	&FastIOInfoPin8,	8,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc8,	MUX_SIZE(MuxDesc8),	FN_GPIO,	-1,	1,	0,	NULL	 };	// Arduino IO8
PinDescription AltPin9 =															        
	{ 4,	NONE,	&FastIOInfoPin9,	9,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc9,	MUX_SIZE(MuxDesc9),	FN_GPIO,	-1,	1,	0,	NULL	 };	// Arduino IO9 or PWM pin #4
PinDescription AltPin11 =															        
	{ 5,	NONE,	&FastIOInfoPin11,	11,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc11,	MUX_SIZE(MuxDesc11),	FN_GPIO,	-1,	1,	0,	NULL	 };	// Arduino IO11 or SPI1_MOSI or PWM pin #6
PinDescription AltPin4 =
	{ 6,	NONE,	&FastIOInfoPin4,	4,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc4,	MUX_SIZE(MuxDesc4),	FN_GPIO,	-1,	1,	0,	NULL	 };	// Arduino IO4
PinDescription AltPin13 =
	{ 7,	NONE,	&FastIOInfoPin13,	13,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc13,	MUX_SIZE(MuxDesc13),	FN_GPIO,	-1,	1,	0,	NULL	 };	// Arduino IO13 or SPI1_SCK
...
//	gpiolib	alias	fastinf	ardid	Initial			FixdSt	ptMuxDesc,		MuxCount		type		Handle	extPU	iAlt	pAlt
	...
	{ 2,	NONE,	NULL,	7,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc7,	MUX_SIZE(MuxDesc7),	FN_GPIO,	-1,	1,	0,	&AltPin7 },	// Arduino IO7
	{ 3,	NONE,	NULL,	8,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc8,	MUX_SIZE(MuxDesc8),	FN_GPIO,	-1,	1,	0,	&AltPin8 },	// Arduino IO8
	...
	{ 13,	NONE,	NULL,	2,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc2,	MUX_SIZE(MuxDesc2),	FN_GPIO,	-1,	1,	0,	&AltPin2 },	// Arduino IO2 or UART1_RXD
	{ 14,	NONE,	NULL,	3,	FN_GPIO_INPUT_HIZ,	NONE,	(mux_sel_t*)&MuxDesc3,	MUX_SIZE(MuxDesc3),	FN_GPIO,	-1,	1,	0,	&AltPin3 },	// Arduino IO3 or UART1_TXD or PWM pin #1
	...
	{ 38,	NONE,	NULL,	NONE,	NONE,			HIGH,	NULL,			0,			FN_SWITCH,	-1,	0,	0,	NULL	 },	// Direction Control for GPIO #2 - Arduino ID IO7
	{ 39,	NONE,	NULL,	NONE,	NONE,			NONE,	NULL,			0,			FN_SWITCH,	-1,	0,	0,	NULL	 },	// Pullup Control for GPIO #2 - Arduino ID IO7
	{ 40,	NONE,	NULL,	NONE,	NONE,			HIGH,	NULL,			0,			FN_SWITCH,	-1,	0,	0,	NULL	 },	// Direction Control for GPIO #3 - Arduino ID IO8
	...
	{ 61,	NONE,	NULL,	NONE,	NONE,			NONE,	NULL,			0,			FN_RESERVED,	-1,	0,	0,	NULL	 },	// PCIe Reset to h/w
	{ 62,	NONE,	NULL,	NONE,	NONE,			NONE,	NULL,			0,			FN_RESERVED,	-1,	0,	0,	NULL	 },	// Wifi Disable reset to h/w
...
TTYUARTClass Serial2(&rx_buffer3, 2, true);	// ttyQRK1 (IO2/3, system console)
